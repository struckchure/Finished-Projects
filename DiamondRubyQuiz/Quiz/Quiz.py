...
class QuizPage(QGroupBox):
    def __init__(self, quiz_topic, username, widget):
        ...
        self.window_layout.addWidget(
            self.stacked_widget, stretch=0, alignment=Qt.AlignCenter
        )
    ...
    def questionPANEL(self):
        ...
        self.question_panel_layout.addWidget(
            self.question_panel_stacked_widget, stretch=0, alignment=Qt.AlignCenter
        )
        ...
        self.main_layout.addWidget(
            self.question_panel_group, stretch=0, alignment=Qt.AlignBottom
        )
        ...
        try:
            ...
            for i in range(0, len(self.question_answers_collection)):
                if len(self.question_answers_collection[i]) != 0:
                    if (
                        self.question_day[i][-1]
                        == self.question_answers_collection[i][-1]
                    ):
                        self.score += 1
    ...
    def question_signal(self):
        try:
            ...
            self.date_day = str(QCalendarWidget().selectedDate()).replace(
                "PyQt5.QtCore.QDate", ""
            )
            ...
            if self.question_topic:
                ...
                if len(self.quiz_ids) != 0:
                    self.quiz_id_ = self.quiz_ids[
                        self.quiz_topics.index(self.question_topic)
                    ]
                    ...
                    for q in self.question_day:
                        self.nav_panel_layout.addWidget(
                            Templates.QuestionButton(
                                self.question_index, self.question_panel_stacked_widget
                            )
                        )
                        self.question_index += 1
