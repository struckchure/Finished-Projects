...
class QuizMasterVerify(QDialog):
    def __init__(self, stacked_widget, credentials):
        ...
        self.window_layout.addWidget(
            self.border_design, stretch=0, alignment=Qt.AlignTop
        )
        ...
        self.main_layout.addWidget(
            self.verify_button, stretch=0, alignment=Qt.AlignCenter
        )
    ...
    def verify_button_slot(self):
        try:
            ...
            if self.username.text() and self.password.text():
                if (self.username.text() == self.username_val) and (
                    self.password.text() == self.password_val
                ):
                    ...
                    self.stacked_widget.addWidget(
                        SetQuiz.QuizSheet(self.stacked_widget)
                    )
                ...
                if (
                    ...
                    if self.username.text() in all_usernames:
                        ...
                        if self.new_password.text() == self.confirm_password.text():
                            ...
                            self.admin_verify = QuizMasterVerify(
                                self.background_stacked_widget, self.credentials
                            )
