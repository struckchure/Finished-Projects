...
class QuizSheet(QGroupBox):
    ...
    def topBAR(self):
        ...
        self.top_bar_layout.addWidget(
            self.add_question, stretch=0, alignment=Qt.AlignLeft
        )
    ...
    def questionLAYOUT(self):
        ...
        self.init_question = Templates.QuestionButton(
            str(self.current_number), self.question_stacked_widget
        )
        self.init_question.setMaximumSize(self.btn_size[0], self.btn_size[1])
        self.top_bar_layout.addWidget(
            self.init_question, stretch=0, alignment=Qt.AlignRight
        )
                ...
                if self.username.text() and self.password.text():
                    if self.username.text() in self.all_usernames:
                        if (
                            self.password.text()
                            == self.all_passwords[
                                self.all_usernames.index(self.username.text())
                            ]
                        ):
    ...
    def submit_slot_helper(self):
        try:
            ...
            for i in range(1, self.current_number):
                self.get_question = self.findChild(
                    QTextEdit, f"question_{i}"
                ).toPlainText()
...
class SetQuiz(QGroupBox):
    ...
    def centralWIDGET(self):
        ...
        self.central_mid_layout.addWidget(
            self.register_quiz_master_button,
            stretch=0,
        ...
        self.background_stacked_widget.addWidget(
            QuizMasterRegistration.QuizMasterRegister()
        )
        self.background_stacked_widget.addWidget(
            QuizSheet(self.background_stacked_widget)
        )
        self.background_stacked_widget.setCurrentIndex(0)
