...
from DiamondRubyQuiz.Tools import Templates, LoopEmit, Database
import sys
...
class ResultTable(QGroupBox):
	def __init__(self, id_, serial_num, quiz_topic, quiz_day, quiz_master, quiz_duration):
		QGroupBox.__init__(self)
...
class DisplayResult(QDialog):
	...
	def distributeRESULT(self):
		try:
			self.dialog_group_layout.addLayout(ResultGrid('0', 'S/N', 'Quiz Topic', 'Candidate', 'Score'))

			number = 1
			self.all_result = Database.get_quiz_results_all(self.quiz_topic)

			for r in self.all_result:
				self.dialog_group_layout.addLayout(ResultGrid(r[0], number, str(r[1]).replace('_', ' '), r[2], r[3]))
		except Exception as e:
			raise e
